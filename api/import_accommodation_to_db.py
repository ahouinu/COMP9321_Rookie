...
def get_mark(suburb):
    ...
    :param suburb: suburb
    :return: dictionary{corresponding dewelling type: max score in four types}
    '''
    ...
    for key in sum_price_dic.keys():
        if sum_price_dic[key] == max(sum_price_dic.values()):
            price_performance_ratio = {key: str(round(max(sum_price_dic.values())/sum(sum_price_dic.values())*100,2))+'%'}
